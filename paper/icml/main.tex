...
\twocolumn[
  ...
  \begin{icmlauthorlist}
    \icmlauthor{Jun Huang}{equal,uestc}
    \icmlauthor{Xin Ren}{equal,uestc}
  \end{icmlauthorlist}
  \icmlaffiliation{uestc}{University of Electronic Science and Technology of China, Chengdu, China}
  \icmlcorrespondingauthor{Jun Huang}{202552280729@std.uestc.edu.cn}
...
Open decision models have followed quickly \citep{kev2026,laya2026,cheng2026thisthat}, together with work on
selective control \citep{wu2026reflex} and on read-out failure modes \citep{sun2026typesafe} that studies how such
models behave inside agents. All of these models learn from
...
We call this procedure \emph{interface distillation} (\cref{fig:overview}).

We build \wev on this observation. A non-generative decision model is trained on three sources: episodes
...
\end{itemize}
Code, data builders, models and evaluation results are public.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_teaser.pdf}
\caption{Browser steps against general decisions (test splits). Open decision models trained on general data stay
below 22\% step success on unseen websites; \wev reaches 68--76\% while remaining competitive out of domain.}
\label{fig:teaser}
\end{figure}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_overview.png}
...
(Illustration drawn with an image-generation model from a written specification and checked by the authors.)}
\label{fig:overview}
\end{figure*}
...
of text; its training method is unpublished. Open models share its interface but differ in backbone and data: decoder models adapted with LoRA
\citep{kev2026,hu2022lora}, bidirectional encoders with a small decision head \citep{laya2026,devlin2019bert,warner2024modernbert},
and a 2B decoder read at a designated position \citep{cheng2026thisthat}. All of them train on general classification,
question-answering and rule data. REFLEX \citep{wu2026reflex} places a hosted decision model inside an
...
The backbone and read-out follow an open-source implementation \citep{kev2026}. The state is written once, and each question follows it as
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_done.pdf}
\caption{Stopping trade-off on the NNetNav test split (382 DONE steps, 768 others). Each curve accepts DONE only when
its probability exceeds a threshold swept from 0.05 to 0.99; circles mark 0.5 and squares 0.8.}
\label{fig:done}
\end{figure}
...
Code, data builders and evaluation results: \url{https://github.com/alanhuangyoo/wev}. Models:
